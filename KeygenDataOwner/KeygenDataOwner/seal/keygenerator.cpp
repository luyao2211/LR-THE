...
namespace seal
{
    KeyGenerator::KeyGenerator(shared_ptr<SEALContext> context, uint64_t *pk1) :
    ...
    KeyGenerator::KeyGenerator(shared_ptr<SEALContext> context,
        const SecretKey &secret_key, uint64_t *pk1) : context_(move(context))
    {
        // Verify parameters
        if (!context_)
        {
    ...
    void KeyGenerator::generate_pk(uint64_t *pk1)
    {
        if (!sk_generated_)
        {
		...
		for (size_t i = 0; i < coeff_count * coeff_mod_count; i++)
		{
			public_key_1[i] = pk1[i];
		}
