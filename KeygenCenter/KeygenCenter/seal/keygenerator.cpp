...
namespace seal
{
	...
	//Éú³Épk1
	KeyGenerator::KeyGenerator(shared_ptr<SEALContext> context, bool flag) :
		context_(move(context))
	{
		...
		pk1_ = public_key_.data().data(1);
		set_poly_coeffs_uniform(context_data, pk1_, random);
	}
	...
	//Êä³öpk1
	uint64_t* &KeyGenerator::public_key_1()
	{
		return pk1_;
	}
