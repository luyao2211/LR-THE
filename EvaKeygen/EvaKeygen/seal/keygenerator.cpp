...
namespace seal
{
	...
	void KeyGenerator::skadd(SecretKey &sk)
	{
		auto &context_data = *context_->context_data();
		auto &parms = context_data.parms();
		auto &coeff_modulus = parms.coeff_modulus();
		...
		{
			// Perform inverse NTT
			inverse_ntt_negacyclic_harvey(sk.data().data() + (i * coeff_count),
				small_ntt_tables[i]);
			inverse_ntt_negacyclic_harvey(secret_key_.data().data() + (i * coeff_count),
				small_ntt_tables[i]);
			add_poly_poly_coeffmod(sk.data().data() + (i * coeff_count),
				secret_key_.data().data() + (i * coeff_count), coeff_count, coeff_modulus[i],
				secret_key_.data().data() + (i * coeff_count));
			// Perform NTT
			ntt_negacyclic_harvey(sk.data().data() + (i * coeff_count), small_ntt_tables[i]);
			ntt_negacyclic_harvey(secret_key_.data().data() + (i * coeff_count), small_ntt_tables[i]);
		}
	}
	...
	void KeyGenerator::combine_relin_keys(RelinKeys &rlk1, RelinKeys rlk2, SecretKey sk1, SecretKey sk2, int decomposition_bit_count)
	{
		// Check that decomposition_bit_count is in correct interval
		if (decomposition_bit_count < SEAL_DBC_MIN ||
			decomposition_bit_count > SEAL_DBC_MAX)
		...
		//读取私钥
		Pointer<uint64_t> secret_key_array1 = allocate_poly(coeff_count, coeff_mod_count, pool_);
		set_poly_poly(sk1.data().data(), coeff_count, coeff_mod_count, secret_key_array1.get());
		Pointer<uint64_t> secret_key_array2 = allocate_poly(coeff_count, coeff_mod_count, pool_);
		set_poly_poly(sk2.data().data(), coeff_count, coeff_mod_count, secret_key_array2.get());
		...
		for (size_t k = 0; k < 1; k++)
		{
			for (size_t l = 0; l < coeff_mod_count; l++)
			{
				...
				for (size_t i = 0; i < decomposition_factors[l].size(); i++)
				{
					uint64_t *eval_keys_first = rlk1.data()[k][l].data(2 * i);
					uint64_t *eva2_keys_first = rlk2.data()[k][l].data(2 * i);
					for (size_t j = 0; j < coeff_mod_count; j++)
					{
						//把两个加在一起
						add_poly_poly_coeffmod(
							eval_keys_first + (j * coeff_count), eva2_keys_first + (j * coeff_count),
							coeff_count, coeff_modulus[j], eval_keys_first + (j * coeff_count));
						// multiply 2 * w^i * s1 * s2
						uint64_t decomposition_factor_mod = decomposition_factors[l][i] &
							static_cast<uint64_t>(-static_cast<int64_t>(l == j));
						multiply_poly_scalar_coeffmod(
							secret_key_array1.get() + (j * coeff_count), coeff_count,
							decomposition_factor_mod * 2, coeff_modulus[j], temp.get());
						dyadic_product_coeffmod(
							temp.get(), secret_key_array2.get() + (j * coeff_count),
							coeff_count, coeff_modulus[j], temp.get());
						// add 2 * w^i * s1 * s2 into relin_keys_[k].first[i]
						add_poly_poly_coeffmod(
							eval_keys_first + (j * coeff_count), temp.get(), coeff_count,
							coeff_modulus[j], eval_keys_first + (j * coeff_count));
					}
				}
			}
    ...
    GaloisKeys KeyGenerator::galois_keys(int decomposition_bit_count, uint64_t *pk1,
	...
	void KeyGenerator::combine_galois_keys(GaloisKeys &gk1, GaloisKeys &gk2, int decomposition_bit_count)
	{
		// Check to see if secret key and public key have been generated
		if (!sk_generated_)
		{
		...
		for (uint64_t galois_elt : logn_galois_keys)
		{
			uint64_t index = (galois_elt - 1) >> 1;
			...
			for (size_t l = 0; l < coeff_mod_count; l++)
			{
				...
				for (size_t i = 0; i < decomposition_factors[l].size(); i++)
				{
					uint64_t *eval_keys_first = gk1.data()[index][l].data(2 * i);
					uint64_t *eva2_keys_first = gk2.data()[index][l].data(2 * i);
					for (size_t j = 0; j < coeff_mod_count; j++)
					{
						//把两个加在一起
						add_poly_poly_coeffmod(
        ...
        const vector<int> &steps)
    {
        ...
        return galois_keys(decomposition_bit_count, pk1, galois_elts);
    }
    ...
    GaloisKeys KeyGenerator::galois_keys(int decomposition_bit_count, uint64_t *pk1)
    {
        ...
        return galois_keys(decomposition_bit_count, pk1, logn_galois_keys);
    }
