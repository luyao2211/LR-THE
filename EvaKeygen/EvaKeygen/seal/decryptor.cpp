...
namespace seal
{
	...
	void Decryptor::skadd(SecretKey &sk)
	{
		auto &context_data = *context_->context_data();
		auto &parms = context_data.parms();
		auto &coeff_modulus = parms.coeff_modulus();
		...
		for (size_t i = 0; i < coeff_mod_count; i++)
		{
			...
			inverse_ntt_negacyclic_harvey(sk.data().data() + (i * coeff_count),
				small_ntt_tables[i]);
			inverse_ntt_negacyclic_harvey(secret_key_array_.get() + (i * coeff_count),
				small_ntt_tables[i]);
			add_poly_poly_coeffmod(sk.data().data() + (i * coeff_count),
				secret_key_array_.get() + (i * coeff_count), coeff_count, coeff_modulus[i],
				sk.data().data() + (i * coeff_count));
			// Perform NTT
			ntt_negacyclic_harvey_lazy(sk.data().data() + (i * coeff_count), small_ntt_tables[i]);
			ntt_negacyclic_harvey_lazy(secret_key_array_.get() + (i * coeff_count), small_ntt_tables[i]);
		}
	}

	void Decryptor::partialdecrypt(Ciphertext &encrypted)
	{
		...
		size_t coeff_mod_count = coeff_modulus.size();
		auto &small_ntt_tables = context_data.small_ntt_tables();
		auto tmp_dest_modq(allocate_zero_poly(coeff_count, coeff_mod_count, pool_));
	...
	void Decryptor::set_poly_coeffs_normal(
