...
namespace seal
{
	...
	void Encryptor::pkadd(PublicKey &pk)
	{
		...
		{
			// Perform inverse NTT
			inverse_ntt_negacyclic_harvey(pk.data().data(0) + (i * coeff_count),
				small_ntt_tables[i]);
			inverse_ntt_negacyclic_harvey(public_key_.get() + (i * coeff_count),
				small_ntt_tables[i]);
			add_poly_poly_coeffmod(pk.data().data(0) + (i * coeff_count),
				public_key_.get() + (i * coeff_count), coeff_count, coeff_modulus[i],
				public_key_.get() + (i * coeff_count));
			// Perform NTT
			ntt_negacyclic_harvey_lazy(pk.data().data(0) + (i * coeff_count), small_ntt_tables[i]);
			ntt_negacyclic_harvey_lazy(public_key_.get() + (i * coeff_count), small_ntt_tables[i]);
		}
	}
